...
namespace abc
{
	template<class Item>
		class sorter
		{
			public:
				...
				void counting_sort(vector<Item>& a,int lo,int hi);
				void bucket_sort_list(vector<Item>& a,int lo,int hi);
				void bucket_sort_queue(vector<Item>& a,int lo,int hi);
				void bucket_sort_stack(vector<Item>& a,int lo,int hi);
	...
	template<class Item>
		void sorter<Item>::counting_sort(vector<Item>& a, int lo, int hi)
		{
			int k = 10000;
			int i,j;
			vector<Item> C(k,0);
			vector<Item> B(hi-lo+1,0);
			for(int i=lo; i<=hi ; i++)
				C[a[i]]++;
			for(int i=1;i<k;i++)
				C[i] = C[i] + C[i-1];
	...
	template <class Item>
		void sorter<Item>::bucket_sort_list(vector<Item>& a,int lo,int hi)
		{
			list<Item> C[10000];
	...
	template <class Item>
		void sorter<Item>::bucket_sort_queue(vector<Item>& a,int lo,int hi)
		{
			queue<Item> C[10000];
	...
	template <class Item>
		void sorter<Item>::bucket_sort_stack(vector<Item>& a,int lo ,int hi)
		{
			stack<Item> C[10000];
			for(int i=lo;i<=hi;i++)
				C[a[i]].push(a[i]);
